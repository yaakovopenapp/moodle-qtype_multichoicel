 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multichoicel extends question_type {
    public function get_question_options($question) {
        global $DB, $OUTPUT;

        $question->options = $DB->get_record('qtype_multichoicel_options', ['questionid' => $question->id]);
    ...
    protected function create_default_options($question) {
        ...
        $config = get_config('qtype_multichoicel');
        ...
        $options->maximumanswers = $config->maximumanswers;
    ...
    public function save_defaults_for_new_questions(stdClass $fromform): void {
        ...
        $this->set_default_value('maximumanswers', $fromform->maximumanswers);
    }
    ...
    public function save_question_options($question) {
        ...
        if ($answercount < 2) { // Check there are at lest 2 answers for multiple choice.
            $result->error = get_string('notenoughanswers', 'qtype_multichoicel', '2');
            return $result;
        }
        ...
        $options = $DB->get_record('qtype_multichoicel_options', array('questionid' => $question->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $question->id;
            ...
            $options->showstandardinstruction = 0;
            $options->maximumanswers = 1;
            $options->id = $DB->insert_record('qtype_multichoicel_options', $options);
        }
        ...
        $options->maximumanswers = $question->maximumanswers;
        $DB->update_record('qtype_multichoicel_options', $options);
        ...
        if ($options->single) {
            if ($maxfraction != 1) {
                $result->noticeyesno = get_string('fractionsnomax', 'qtype_multichoicel',
                        $maxfraction * 100);
                return $result;
            }
        } else {
            ...
            if ($totalfraction != 1) {
                $result->noticeyesno = get_string('fractionsaddwrong', 'qtype_multichoicel',
                        $totalfraction * 100);
                return $result;
            }
        }
    ...
    protected function make_question_instance($questiondata) {
        ...
        if ($questiondata->options->single) {
            $class = 'qtype_multichoicel_single_question';
        } else {
            $class = 'qtype_multichoicel_multi_question';
        }
        return new $class();
    }
    ...
    protected function initialise_question_instance(question_definition $question, $questiondata) {
        ...
        $question->maximumanswers = $questiondata->options->maximumanswers;
        if (!empty($questiondata->options->layout)) {
            $question->layout = $questiondata->options->layout;
        } else {
            $question->layout = qtype_multichoicel_single_question::LAYOUT_VERTICAL;
        }
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('qtype_multichoicel_options', array('questionid' => $questionid));
     ...
     *      should be a lang string answernumberingxxx in teh qtype_multichoicel
    ...
    public static function get_numbering_styles() {
        ...
        foreach (array('abc', 'ABCD', '123', 'iii', 'IIII', 'none') as $numberingoption) {
            $styles[$numberingoption] =
                    get_string('answernumbering' . $numberingoption, 'qtype_multichoicel');
        }
        return $styles;
    }

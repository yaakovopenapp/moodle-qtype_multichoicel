...
use qtype_multichoicel\privacy\provider;
...
global $CFG;
require_once($CFG->dirroot . '/question/type/multichoicel/classes/privacy/provider.php');
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('qtype_multichoicel');
        $actual = \qtype_multichoicel\privacy\provider::get_metadata($collection);
    ...
    public function test_export_user_preferences($name, $value, $expected) {
        $this->resetAfterTest();
        $user = $this->getDataGenerator()->create_user();
        set_user_preference("qtype_multichoicel_$name", $value, $user);
        provider::export_user_preferences($user->id);
        $writer = writer::with_context(\context_system::instance());
        $this->assertTrue($writer->has_any_data());
        $preferences = $writer->get_user_preferences('qtype_multichoicel');
        foreach ($preferences as $key => $pref) {
            $preference = get_user_preferences("qtype_multichoicel_{$key}", null, $user->id);
            if ($preference === null) {
                continue;
            }
